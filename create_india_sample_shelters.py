#!/usr/bin/env python3
"""
Create sample shelter data for Indian mountain areas (for testing).
This simulates data that would come from Google Maps API.
"""
...
# Coordinates for Uttarakhand (Dehradun area) - popular mountain region
sample_shelters = [
...
output_file = f'shelters_{timestamp}.xlsx'

# Save to Excel
df.to_excel(output_file, index=False, sheet_name='All Shelters')

print("="*60)
print("Sample Indian Mountain Area Shelter Data Created")
print("="*60)
print(f"✓ Created: {output_file}")
print(f"✓ Total shelters: {len(df)}")
print(f"\nBreakdown:")
print(f"  - Schools: {len(df[df['type'] == 'schools'])}")
print(f"  - Police Stations: {len(df[df['type'] == 'police_stations'])}")
print(f"  - Fire Stations: {len(df[df['type'] == 'fire_stations'])}")
print(f"\nRegions covered:")
print(f"  - Dehradun (Uttarakhand)")
print(f"  - Mussoorie (Uttarakhand)")
print(f"  - Rishikesh (Uttarakhand)")
print(f"  - Haridwar (Uttarakhand)")
print(f"\nThis file will be auto-loaded by the Flask app on startup.")
print("="*60)
